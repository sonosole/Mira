...
"""
    znorm(x::Variable{T}; dims::IntOrDims{N}=1) where {T,N}
...
"""
function znorm(x::Variable{T}; dims::IntOrDims{N}=1) where {T,N}
    D = eltype(x)
    l = D(1)
    ϵ = D(1e-38)
...
function znorm(x::AbstractArray; dims::IntOrDims{N}=1) where N
    l = eltype(x)(1)
    ϵ = eltype(x)(1e-38)
    μ = mean(x; dims)
